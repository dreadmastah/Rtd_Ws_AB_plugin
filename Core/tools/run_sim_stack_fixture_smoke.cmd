...
python -c "import json, pathlib; p=pathlib.Path(r'%RISKVIEW_JSON%'); assert p.exists(); o=json.loads(p.read_text(encoding='utf-8')); assert o['messageType']=='AccountRiskView.v1'; assert o['orderRoutingEnabled'] is False; assert o['sourceFresh'] is True; assert pathlib.Path(r'%RISKVIEW_HTML%').exists(); h=pathlib.Path(r'%RISKVIEW_HTML%').read_text(encoding='utf-8'); assert 'READ ONLY' in h and 'No order controls exist in this view.' in h; print('ACCOUNT_RISK_VIEW_SUPERVISED=PASS')"
set RC=%ERRORLEVEL%
