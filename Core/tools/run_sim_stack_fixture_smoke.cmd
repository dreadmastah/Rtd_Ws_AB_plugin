...
python -c "import json, pathlib; p=pathlib.Path(r'%RISKVIEW_JSON%'); assert p.exists(); o=json.loads(p.read_text(encoding='utf-8')); a=o['accountRiskObservation']; s=o['symbolRisk']; rows={r['symbol']:r for r in s['symbols']}; assert o['messageType']=='AccountRiskView.v1'; assert o['orderRoutingEnabled'] is False; assert o['sourceFresh'] is True; assert a['ready'] is True; assert a['riskState']=='NORMAL'; assert abs(a['riskCapital']-10250.0)<1e-9; assert abs(a['availableBalance']-9150.0)<1e-9; assert abs(a['grossNotional']-1000.0)<1e-9; assert abs(a['projectedGrossNotional']-1000.0)<1e-9; assert abs(a['marginBalance']-10250.0)<1e-9; assert abs(a['initialMargin']-100.0)<1e-9; assert abs(a['projectedEffectiveLeverage']-(1000.0/10250.0))<1e-9; assert abs(a['projectedMarginUtilization']-(100.0/10250.0))<1e-9; assert abs(a['projectedNetDirectionalNotional']-1000.0)<1e-9; assert s['ready'] is True; assert len(rows)==12; assert rows['BTCUSDT']['positionMode']=='LONG'; assert abs(rows['BTCUSDT']['currentNotional']-1000.0)<1e-9; assert rows['ETHUSDT']['positionMode']=='FLAT'; assert abs(rows['ETHUSDT']['currentNotional'])<1e-9; assert pathlib.Path(r'%SYMBOLRISK%').exists(); raw=json.loads(pathlib.Path(r'%SYMBOLRISK%').read_text(encoding='utf-8')); assert raw['orderRoutingEnabled'] is False; assert len(raw['symbols'])==12; assert pathlib.Path(r'%RISKVIEW_HTML%').exists(); h=pathlib.Path(r'%RISKVIEW_HTML%').read_text(encoding='utf-8'); assert 'READ ONLY' in h and 'No order controls exist in this view.' in h and 'Live account and projected headroom' in h and 'Per-symbol projected exposure' in h; print('ACCOUNT_RISK_VIEW_SUPERVISED=PASS')"
set RC=%ERRORLEVEL%
